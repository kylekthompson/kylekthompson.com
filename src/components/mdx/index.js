...
function highlightsFromMetastring(metastring) {
  ...
  if (!highlightsMatch) { return []; }

  const highlights = highlightsMatch.groups.highlights;

  return highlights.split(',').map((highlight) => highlight.includes('-') ? highlight : parseInt(highlight, 10));
}

function codePropsFromPreProps(preProps) {
  // look for <pre><code>some code string in here</code></pre>
  if (preProps.children && preProps.children.props && preProps.children.props.name === 'code') {
    const {
      children: code,
...
export default {
  ...
  h4: props => <H5 {...props} />,
  h5: props => <H6 {...props} />,
  h6: () => { throw new Error('Cannot nest heading that deep.') },
  p: props => <P {...props} />,
  ul: props => <UL {...props} />,
  ol: props => <OL {...props} />,
  pre: preProps => {
